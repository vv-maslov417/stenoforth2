...
\ if addr2 = 0 continue till RET instruction
               ...
               \ BASE @ >R DECIMAL . R> BASE !  ." bytes)"
                ...
                REPEAT ." END-CODE " CR
                2DUP NIP
                ." ( " BASE @ DECIMAL  SWAP R> - ABS . BASE !  ." bytes, "
                 2DROP
               \ ."              ("
                BASE @ DECIMAL CNT-INST . BASE !  ." instructions )" CR
                ;
...
: auc dup | a\ A\ |
  begin A DIS-OP -> A
    A c@ 0xC3 = if a A a - 1+  exit
    else A w@ 0x00 = A c@ 0x55 = A c@ 0xAF = or or
         if a A a - exit then
    then
  again
;
...
: pairs-chars u\ a\ 0 dpo\ here bfo\ u 2* allot
  a u ado i c@ bl >
          if i w@ bfo dpo + w! dpo 3 + -> dpo 2 else 1 then
      +loop bfo dpo 1- ado i 2 number? 2drop 3 +loop ;
: st-dis hex load-str ;
: sees \ see for data-stack
  pairs-chars
  depth 0 do i roll loop here >r
  depth 0 do c,     loop r>
  dup 0 | a\ A\ n\ |
